...
# CREATE THE SCRIPT WHICH IS RUNNED ON SUDO like sudo node ...
sudo tee /usr/bin/open5gs-webui-start.sh > /dev/null <<EOF
#!/bin/bash
...
ExecStart=/usr/bin/open5gs-webui-start.sh
